    ...
    try {
      ...
      // --- 处理其他动作 (app, url, etc.) --- 
        ...
        results.push(await new Promise((resolve) => {
           ...
           let script = `
             set closed_apps to {}
             set not_running_apps to {}
             set error_apps to {}

             repeat with browserName in {${browsersToClose.map(b => `"${b}"`).join(', ')}}
               try
                 tell application "System Events"
                   if (name of processes) contains browserName then
                     tell application browserName to quit
                     delay 0.5 -- Give time for the app to quit
                     set end of closed_apps to browserName
                   else
                     set end of not_running_apps to browserName
                   end if
                 end tell
               on error errMsg number errNum
                 log "Error quitting " & browserName & ": " & errMsg & " (" & errNum & ")"
                 set end of error_apps to browserName
               end try
             end repeat

             set summary to "Closed: " & (closed_apps as text)
             if (count of not_running_apps) > 0 then
               set summary to summary & ". Not running: " & (not_running_apps as text)
             end if
             if (count of error_apps) > 0 then
                set summary to summary & ". Errors quitting: " & (error_apps as text)
             end if
             return summary
           `;
           runAppleScript(script.trim(), (err, result) => {
              const success = !err;
